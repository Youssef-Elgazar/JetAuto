...
- [ros2_ws/src/driver/controller/launch/controller.launch.py](/home/ubuntu/ros2_ws/src/driver/controller/launch/controller.launch.py) provides the chassis controller used by the app nodes that publish `/controller/cmd_vel`. **Confirmed not currently included in `bringup.launch.py`** — this is why `/controller` didn't appear in `ros2 node list` during motion debugging. See "Motion debugging notes" below.
...
## Motion debugging notes — RESOLVED (direct motor path)

- ~~The most direct motion repro is [Scripts/jetauto_forward_probe.py](Scripts/jetauto_forward_probe.py), which publishes a short forward Twist to `/controller/cmd_vel` and then stops.~~ **This probe will not work as-is** — `/controller/cmd_vel` has no subscriber right now, because `controller.launch.py` (which provides `odom_publisher_node.py`, the actual `cmd_vel` consumer) is not included in the current `bringup.launch.py`. Confirmed via `ros2 node list` showing no `/controller` node.
- **Confirmed working alternative**: direct motor control via `/ros_robot_controller/set_motor` (`ros_robot_controller_msgs/msg/MotorsState`). This bypasses `controller.launch.py` entirely and talks straight to the board-facing node.
```bash
  ros2 topic pub --once /ros_robot_controller/set_motor \
    ros_robot_controller_msgs/msg/MotorsState "{data: [{id: 1, rps: 1.0}]}"
```
  `rps: 0.0` stops the motor.
...
- **The serial/controller boundary is no longer the blocker.** The actual issue was that the `ros_robot_controller` node launched via bringup can end up alive (serial open, spinning) without registering on the ROS graph; fix is restarting the whole `bringup.launch.py` process, not the individual node.
- **Next step for `cmd_vel`-based scripts (`jetauto_forward_probe.py`, `jetauto_wasd_qe_teleop.py`, joystick control)**: launch [ros2_ws/src/driver/controller/launch/controller.launch.py](/home/ubuntu/ros2_ws/src/driver/controller/launch/controller.launch.py) alongside bringup (or add it into `bringup.launch.py`'s includes) so `odom_publisher_node.py` is actually running and subscribed to `controller/cmd_vel` / `cmd_vel`. Then re-test the Twist-based probe.